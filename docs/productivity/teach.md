...
```bash
npx skills add galosandoval/skills --skill=teach
```
...
[Source](../../skills/productivity/teach)
...
Two honest notes on that list. A glossary suits most topics, but the skill ships a `GLOSSARY-FORMAT.md` that `SKILL.md` no longer links to, so you will only get one if you ask ([issue #559](https://github.com/galosandoval/skills/issues/559)). And the workspace is not always created where you expect — see the first question below before you build a long course on top of it.
