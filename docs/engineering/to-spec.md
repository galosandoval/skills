...
Its neighbours upstream are [grill-with-docs](./grill-with-docs.md), which does the deciding this skill only records, and [wayfinder](./wayfinder.md), whose finished map merges onto the chain right here. Downstream, [to-tickets](./to-tickets.md) cuts the spec into tracer-bullet tickets for [implement](./implement.md) to build. When you're unsure which skill or flow fits, [ask-galo](./ask-galo.md) routes you.
