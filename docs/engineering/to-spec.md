...
```bash
npx skills add galosandoval/skills --skill=to-spec
```
...
`to-spec` turns the conversation you have just had into a **[spec](https://www.aihero.dev/ai-coding-dictionary/spec)**, and writes it to a markdown file in the repo — `.scratch/<feature-slug>/spec.md`, or a path you pass it.
...
None. `to-spec` writes a file into the repo, so it does not need a tracker configured. It sharpens the spec with your `CONTEXT.md` glossary and ADRs when they exist, and works without them. Publishing to a tracker is [to-tickets](./to-tickets.md)' job, and that one does need [setup-skills](./setup-skills.md) to have run.
...
**Why doesn't the spec go on my issue tracker any more?**
Because a spec is a document, not a unit of work. As an issue it collected a `ready-for-agent` label it did not deserve, and [AFK](https://www.aihero.dev/ai-coding-dictionary/afk) agents polling that label would try to build the whole spec in one run instead of picking up the ticket slices. Publishing is now [to-tickets](./to-tickets.md)' job alone: `to-spec` writes the file, `to-tickets` turns it into issues that are actually grabbable. If you want the spec visible to collaborators, commit it or paste it into an issue yourself.

**Where does the file land, and should I commit it?**
`.scratch/<feature-slug>/spec.md` by default — the same directory [to-tickets](./to-tickets.md) uses for local tickets — or wherever you point it: `/to-spec docs/specs/billing.md`. Whether `.scratch/` is committed is your call; it is a throwaway artifact by design (see below), so most people gitignore it and commit only the specs they want reviewed.
...
**Do I still have to run `/to-tickets` in the same context window?**
It is no longer load-bearing. The spec is a file, so if you do [clear](https://www.aihero.dev/ai-coding-dictionary/clearing) or [compact](https://www.aihero.dev/ai-coding-dictionary/compaction), pass the path — `/to-tickets .scratch/<feature>/spec.md` — and it reads the whole thing off disk rather than fetching a truncated issue body back. Running both in one window is still the cheapest path when you have the room.

## It's working if

- It starts writing rather than asking you a fresh round of questions.
- It hands you back a file path, and creates no issues.
...
Its neighbours upstream are [grill-with-docs](./grill-with-docs.md), which does the deciding this skill only records, and [wayfinder](./wayfinder.md), whose finished map merges onto the chain right here. Downstream, [to-tickets](./to-tickets.md) reads the spec file, cuts it into tracer-bullet tickets and publishes those to the tracker for [implement](./implement.md) to build — the split is deliberate: this skill writes a document, `to-tickets` owns the tracker. When you're unsure which skill or flow fits, [ask-galo](./ask-galo.md) routes you.
