...
```bash
npx skills add galosandoval/skills --skill=to-tickets
```
...
That was the old failure mode, when the spec lived in a tracker issue that served back a truncated body. [to-spec](./to-spec.md) now writes the spec to a file — `.scratch/<feature-slug>/spec.md` by default — so pass the path and it reads the whole thing off disk. Running both in one context window, without [clearing](https://www.aihero.dev/ai-coding-dictionary/clearing) or [compacting](https://www.aihero.dev/ai-coding-dictionary/compaction) between them, is still the cheapest path when you have the room.
...
Upstream is [to-spec](./to-spec.md), which hands it a settled spec file to slice against — `to-spec` writes the document, this skill owns the tracker. Downstream is [implement](./implement.md), which builds one ticket per fresh session, driving [tdd](./tdd.md) for the tests and closing with [code-review](./code-review.md). When you're unsure which skill or flow fits, [ask-galo](./ask-galo.md) routes you.
