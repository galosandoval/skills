...
Fill the template below. Its links are written as they should appear **on the finished page**, so they resolve from `docs/<bucket>/` — not from this file. The **fixed frame** (Quickstart block, source link, `## What it does`, `## When to reach for it`, `## Where it fits`) appears on every page. The **adaptable middle** — `## Prerequisites` and the free-form substance sections — carries only what this particular skill earns; delete the rest.
...
- Every link into this repo is relative, and every one resolves.
