...
Act whenever a promoted skill is added, renamed, or has its behaviour changed: create or re-sync its docs page. A rename moves the file too (`docs/<bucket>/<old>.md` → `docs/<bucket>/<new>.md`), because the file name tracks the skill name, and every relative link pointing at it has to move with it; a skill that moves between `engineering/` and `productivity/` moves its docs file to the matching folder. Skills in `misc/`, `personal/`, `in-progress/`, and `deprecated/` get no page — none of those buckets is promoted. A skill moving *out* of one of them into `engineering/` or `productivity/` gains a page; one moving the other way loses it.

Because these pages are repo-only, **every link is repo-relative** — never an absolute URL into this repo. A link to another skill's page in the same bucket is `./<name>.md`; across buckets it is `../<bucket>/<name>.md`; a link into the rest of the repo is relative too (`../../skills/<bucket>/<name>`). Absolute URLs are for genuinely external destinations only. Anything relative keeps working on a fork, a branch, or a local checkout.

There is no H1 — the file name is the page's identity.

## Page structure

Fill the template below, keeping its order. Its links are written as they should appear **on the finished page**, so they resolve from `docs/<bucket>/` — not from this file. The **fixed frame** (Quickstart block, source link, `## What it does`, `## When to reach for it`, `## Where it fits`) appears on every page. `## Prerequisites` and the free-form substance sections carry only what this particular skill needs; delete the rest.
...
**A page carries its own install block.** Upstream drops it because the ai-hero page template renders an install widget above the body; this fork publishes nowhere, so there is no widget and a page without the block leaves the reader no way to install the skill. Keep the Quickstart pair and the source link at the top, worded as below.
...
```bash
npx skills add galosandoval/skills --skill=<name>
```
...
Optional — include only when the skill needs something in place to be functional; omit the heading entirely otherwise. Covers: a **workspace it writes into** (a stateful skill like `grill-with-docs` writes `CONTEXT.md` and ADRs; `teach` builds a whole directory — say what it writes and where), **prior setup** (`triage`/`to-spec`/`to-tickets` need `setup-skills` to have configured an issue tracker), or **repo-specific tooling**. A stateless skill that runs anywhere has no prerequisites — drop the section.
...
- **This repo's issues.** `gh issue list --repo galosandoval/skills --search "<skill-name>" --state all`. A question filed twice is a question the page owes an answer to.
...
- **The map.** Point to [ask-galo](../engineering/ask-galo.md), the router over the whole set, so this page stays a node and never has to redraw the graph.
...
- The page carries the Quickstart block and a relative source link, both naming the correct bucket and skill.
...
- `## Where it fits` names the role and links to `ask-galo`.
...
- Every link into this repo is relative, and every one resolves; only genuinely external destinations are absolute.
