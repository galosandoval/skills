...
The plugin is Claude Code only. Everywhere else, [skills.sh](https://skills.sh/galosandoval/skills) copies editable skill files into the project. Use the whole-set form on `README.md`:
...
```bash
npx skills@latest add galosandoval/skills --skill=<name>
```
...
`.claude-plugin/marketplace.json` makes the repo its own single-plugin marketplace (`/plugin marketplace add galosandoval/skills`, then `/plugin install galosandoval-skills@galosandoval`). The official listing supersedes it. It is kept as a fallback for installing the repo directly — an unreleased commit, or a fork — and is **not** documented to users.
